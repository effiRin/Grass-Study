...
- JSP  
    - JSP는 HTML안에 자바 코드가 포함된 것으로 `서블릿 기반의 서버 사이드 스크립트 기술`이다.  

    - 따라서 JSP는 실행할 때 WAS(Web Application Server)에 의해 **서블릿 클래스로 변환된 후 실행**되며, 기능적인 측면에서는 서블릿과 거의 유사하다고 볼 수 있다. 

    - 하지만 HTML 표준에 따라 작성되므로 서블릿과 달리 **웹페이지 작성이 편리**하다는 장점이 있다. 

    <br>

    - 그러나 이러한 변환 과정은 오직 **한번만** 일어난다.  
     따라서 같은 페이지에 수많은 사용자의 요청이 있더라도 처리속도는 거의 떨어지지 않게 된다.   
    - 만약 JSP페이지의 코딩 내용이 변경된다면 다시 변환 과정을 거치는데, 서블릿과 달리 **WAS에서 자동으로 빌드**하고 클라이언트에 화면을 동적으로 보여주므로 개발이 편리하다.
